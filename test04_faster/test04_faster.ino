...
const int minAbsThreshold   = 20;    // کف مطلق ساده برای مقیاس 0..1024 (کمی کمتر برای واکنش سریع‌تر)
...
// Brake options (L298N)
bool electricBrakeEnabled     = true;    // پیش‌فرض سریع‌تر: ترمز الکتریکی
unsigned long electricBrakeMs = 140;     // مدت ترمز الکتریکی کوتاه
bool centerOnBrake            = false;   // سروو را موقع ترمز سنتر نکن (اختیاری برای سرعت بیشتر)
...
public:
    ...
    bool getStableDetection(bool currentDetection) {
        // اگر وضعیت خام تغییر کرد، شروع پنجره جدید
        if (currentDetection != pendingState) {
            pendingState = currentDetection;
            pendingStart = millis();
        } else {
            // اگر هنوز تغییر پایدار نشده، بررسی کن آیا پنجره‌اش پر شده
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (signalStrength > threshold) {
            ...
            if (signalStrength > result.maxSignalStrength) {
                ...
                result.primaryColumn = i;
            }
        }
    }

    if (result.detected && sumStrength > 0) {
        int centroid = (sumWeightedCol + sumStrength / 2) / sumStrength;
        result.primaryColumn = centroid;
    }
...
void setup() {
    Serial.begin(115200);
    Serial.println("\n========= Opal Robot Starting =========");
    Serial.println("Version: Fast Reaction");
    Serial.println("=====================================");
    ...
    // افزایش نرخ نمونه‌برداری؛ اگر نسخه کتابخانه‌ات پشتیبانی نکرد، این خطوط را کامنت کن
